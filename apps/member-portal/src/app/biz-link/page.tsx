...
export default function MemberBizLinkPage() {
  ...
  const [marketProjects, setMarketProjects] = useState<any[]>([]); // Thêm State lưu Chợ dự án
  
  const UPGRADE_URL = "/upgrade"; 

  // --- STATE CHO FORM ĐẮP THỊT ---
  ...
  useEffect(() => {
    const fetchUserAndProjects = async () => {
      ...
      if (profile) {
        ...
        // HÚT SONG SONG DỮ LIỆU "DỰ ÁN CỦA TÔI" VÀ "CHỢ DỰ ÁN MỞ THẦU"
  ...
  // --- THUẬT TOÁN TỰ ĐỘNG NÉN ẢNH (CANVAS API) ---
  ...
  // --- HÀM XỬ LÝ SUBMIT TẠO MỚI LÊN DATABASE ---
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             ...
             <div className="space-y-6">
                <h3 className="text-xl font-black text-slate-800 flex items-center gap-2 border-b border-slate-200 pb-4">
                  <i className="ph-fill ph-storefront text-[#002D62] text-2xl"></i> Dự án đang Mở Thầu
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {marketProjects.length === 0 ? (
                    <div className="col-span-full py-20 bg-slate-50 border border-slate-200 rounded-3xl text-center flex flex-col items-center">
                      <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mb-6 shadow-sm"><i className="ph-fill ph-folder-dashed text-4xl text-slate-300"></i></div>
                      <p className="text-slate-500 font-bold text-lg">Hiện tại chưa có dự án nào đang mở thầu trên hệ thống.</p>
                      <p className="text-slate-400 mt-2">Hãy quay lại kiểm tra sau nhé!</p>
                    </div>
                  ) : (
      ...
      {/* TAB 2: MY PROJECTS (DỮ NGUYÊN FORM ĐĂNG KÝ VÀ DANH SÁCH) */}
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="col-span-2 space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Tên Dự án / Gói thầu (*)</label><input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-900 outline-none focus:border-blue-400" placeholder="VD: Tìm thầu phụ..." /></div>
                        
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Lĩnh vực</label>
                          <div className="flex flex-col gap-2">
                            <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-900 outline-none cursor-pointer focus:border-blue-400"><option value="CONSTRUCTION">Thi công (Construction)</option><option value="DESIGN">Thiết kế (Design)</option><option value="MATERIAL">Cung cấp vật tư (Material)</option><option value="OTHER">Khác (Tự nhập...)</option></select>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {formData.project_type === 'OTHER' && <input type="text" value={formData.custom_project_type} onChange={e => setFormData({...formData, custom_project_type: e.target.value})} className="w-full h-12 px-4 bg-blue-50 border border-blue-200 rounded-xl text-sm font-bold text-blue-900 outline-none" placeholder="Nhập loại công trình..." autoFocus />}
                          </div>
                        </div>
                    ...
                    <div className="bg-slate-50 p-6 md:p-8 rounded-[2rem] border border-slate-100">
                      ...
                      <div className="grid grid-cols-1 gap-6">
                        <div className="space-y-3 bg-white p-5 rounded-2xl border border-slate-200">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer w-fit group"><input type="checkbox" checked={formData.is_investor_hidden} onChange={e => setFormData({...formData, is_investor_hidden: e.target.checked})} className="w-5 h-5 rounded text-[#002D62] cursor-pointer" /><span className="text-sm font-bold text-slate-600 select-none">Ẩn tên Chủ đầu tư (Bảo mật thông tin dự án)</span></label>
                        </div>
                        <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Mô tả Yêu cầu chi tiết (Scope of work)</label><textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full h-32 p-4 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-900 outline-none resize-none focus:border-blue-400" /></div>
                        <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Yêu cầu năng lực Nhà thầu (Requirements)</label><textarea value={formData.requirements} onChange={e => setFormData({...formData, requirements: e.target.value})} className="w-full h-24 p-4 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-900 outline-none resize-none focus:border-blue-400" /></div>
                      </div>
                    </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {myProjects.length === 0 ? (
                    ...
                    myProjects.map(p => (
                      <Link href={`/biz-link/${p.id}`} key={p.id} className="block bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col hover:border-blue-300 hover:shadow-md transition-all group relative overflow-hidden cursor-pointer">
                        <div className="flex justify-between items-start mb-4 relative z-10">
